...
export const getLabels = (locale?: 'en' | 'es') => {
  return locale === 'es' ? {
    ...
    proteinMinimum: "MÍNIMO DE PROTEÍNA",
    carbsFat: "CARB / GRASA",
    target: "OBJETIVO",
    hitThisFirst: "ALCANZA ESTO PRIMERO",
    fuelBalance: "BALANCE DE ENERGÍA",
    planLogicTitle: "LÓGICA DE TU PLAN",
    energyEquationTitle: "TU ECUACIÓN DE ENERGÍA",
    baseMetabolic: "METABOLISMO BASAL",
    totalDailyEnergy: "ENERGÍA DIARIA TOTAL",
    yourDailyTarget: "TU OBJETIVO DIARIO",
    macroBreakdownTitle: "DESGLOSE DE MACROS",
    ...
    proteinMinimum: "PROTEIN MINIMUM",
    carbsFat: "CARBS / FAT",
    target: "TARGET",
    hitThisFirst: "HIT THIS FIRST",
    fuelBalance: "FUEL BALANCE",
    planLogicTitle: "YOUR PERSONALIZED PLAN LOGIC",
    energyEquationTitle: "YOUR ENERGY EQUATION",
    baseMetabolic: "BASE METABOLIC",
    totalDailyEnergy: "TOTAL DAILY ENERGY",
    yourDailyTarget: "YOUR DAILY TARGET",
    macroBreakdownTitle: "MACRO BREAKDOWN & LOGIC",
...
export const PageFooter = ({ pageNumber, totalPages, minimal }: { pageNumber: number; totalPages?: number; minimal?: boolean }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.gold, fontFamily: TYPOGRAPHY.heading }}>METAMORFIT ALPHA</div>
...
export const MacroGrid = ({ metabolicProfile }: { metabolicProfile: MetabolicProfile }) => {
  ...
  const labels = getLabels(metabolicProfile.locale);
  const macros = [
    { label: labels.protein.toUpperCase(), val: metabolicProfile.proteinGrams, icon: ICONS.protein(COLORS.gold), pct: Math.round((metabolicProfile.proteinGrams * 4 / targetCal) * 100), multiplier: 4, color: COLORS.gold },
    { label: labels.carbohydrates.toUpperCase(), val: metabolicProfile.carbsGrams, icon: ICONS.carb(COLORS.teal), pct: Math.round((metabolicProfile.carbsGrams * 4 / targetCal) * 100), multiplier: 4, color: COLORS.teal },
    { label: labels.fats.toUpperCase(), val: metabolicProfile.fatsGrams, icon: ICONS.fat(COLORS.bone), pct: Math.round((metabolicProfile.fatsGrams * 9 / targetCal) * 100), multiplier: 9, color: COLORS.bone },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', gap: SPACING.cellPadding, width: '100%' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.boneMuted }}>{labels.dailyCalories}</div>
        ...
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, color: COLORS.boneMuted }}>{labels.target}</div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', borderLeft: `1px solid ${COLORS.divider || ""}`, borderRight: `1px solid ${COLORS.divider || ""}` }}>
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.boneMuted }}>{labels.proteinMinimum}</div>
        ...
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, color: COLORS.boneMuted }}>{labels.hitThisFirst}</div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.boneMuted }}>{labels.carbsFat}</div>
        ...
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, color: COLORS.boneMuted }}>{labels.fuelBalance}</div>
      </div>
    </div>
  );
};
  ...
  <PageWrapper pageNumber={1 || ""} totalPages={totalPages || ""}>
    ...
    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', borderBottom: `1px solid ${COLORS.teal || ""}`, paddingBottom: SPACING.cellPadding, marginBottom: SPACING.block }}>
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 700, color: COLORS.teal, letterSpacing: 1 }}>METAMORFIT ALPHA</div>
...
export const Page2 = ({ identity, metabolicProfile, notes, totalPages = 3 }: { identity: Identity; metabolicProfile: MetabolicProfile; notes: IntelligenceNote[]; totalPages?: number }) => {
  ...
  const labels = getLabels(identity.locale);

  return (
    <PageWrapper pageNumber={2 || ""} totalPages={totalPages || ""} compact>
      <PageHeader title={labels.planLogicTitle} pageNumber={2 || ""} />

      <Card headerTitle={labels.energyEquationTitle} style={{ marginBottom: SPACING.blockCompact }}>
        <div style={{ display: 'flex', flexDirection: 'row', gap: SPACING.cellPaddingCompact, width: '100%' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {ICONS.flame()}
            <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.boneMuted, marginTop: 4 }}>{labels.baseMetabolic}</div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', borderLeft: `1px solid ${COLORS.goldDivider || ""}`, borderRight: `1px solid ${COLORS.goldDivider || ""}` }}>
            ...
            <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 600, color: COLORS.boneMuted, marginTop: 4 }}>{labels.totalDailyEnergy}</div>
            ...
            <Badge label={labels.maintenance} variant="outline" />
          </div>
          <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: isDeficit ? 'rgba(20, 184, 166, 0.05)' : 'rgba(201, 168, 76, 0.05)', borderRadius: 8, padding: '8px 0' }}>
            ...
            <div style={{ display: 'flex', fontSize: TYPOGRAPHY.scale.label, fontWeight: 700, color: COLORS.teal, marginTop: 4 }}>{labels.yourDailyTarget}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: SPACING.cellPaddingCompact, marginBottom: SPACING.blockCompact, height: 380 }}>
        ...
        <Card headerTitle={labels.macroBreakdownTitle} style={{ flex: 1.4, marginBottom: 0 }}>
