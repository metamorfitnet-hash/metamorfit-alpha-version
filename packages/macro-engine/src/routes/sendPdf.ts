		...
		try {
			...
			if (!payload.explanation && payload.identity && payload.metabolicProfile) {
				...
				const aiResult = await getAiExplanation(env, {
					...
					personalizationScore: (payload as any).personalization?.personalizationScore || 100
				} as any);
			...
			try {
				...
				await createSystemeContact(env, { email, fullName, tags: (payload.tags || []).map(String) });
				...
				Sentry.withScope((scope) => {
					...
					scope.setContext('crm_details', { email, fullName, error: crmErr.message } as any);
